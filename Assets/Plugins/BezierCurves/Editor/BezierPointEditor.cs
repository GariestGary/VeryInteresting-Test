...
namespace BezierCurves
{
	...
	public class BezierPointEditor : Editor
	{
		...
		public override void OnInspectorGUI()
		{
			...
			BezierPoint.HandleStyle newHandleType =
		...
		void OnSceneGUI()
		{
			...
			Vector3 newPosition = Handles.FreeMoveHandle(point.position, point.transform.rotation,
		...
		private static void HandleConnected(BezierPoint p)
		{
			...
			Vector3 newGlobal1 = Handles.FreeMoveHandle(p.globalHandle1, p.transform.rotation,
			...
			Vector3 newGlobal2 = Handles.FreeMoveHandle(p.globalHandle2, p.transform.rotation,
		...
		private static void HandleBroken(BezierPoint p)
		{
			...
			Vector3 newGlobal1 = Handles.FreeMoveHandle(p.globalHandle1, Quaternion.identity,
				HandleUtility.GetHandleSize(p.globalHandle1) * 0.15f, Vector3.zero, Handles.SphereHandleCap);
			Vector3 newGlobal2 = Handles.FreeMoveHandle(p.globalHandle2, Quaternion.identity,
				HandleUtility.GetHandleSize(p.globalHandle2) * 0.15f, Vector3.zero, Handles.SphereHandleCap);
